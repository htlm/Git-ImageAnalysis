...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function fnCanaryRGB_remove_t0001(expPath, contrastVals, imagesToSkip)
...
dataG   = strcat(filenameArray{2}, startImString, '.mat');
load(data);
load(dataG);
...
canaryBkgnd1Red     = hRed0;
canaryBkgnd1Green   = hGreen0;
...
for i = imageRange(1):imageRange(2)
    if isempty(imagesToSkip) || isempty(imagesToSkip == i) % If this image is NOT being skipped
        ...
        load(dataNameG);
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        %%%%    Subtract Background from t0001 image
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        hRed = hRed - canaryBkgnd1Red;
        hGreen = hGreen - canaryBkgnd1Green;
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        %%%%    Coallate Data
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
expRescale  = uint16(expDouble  ./ maxRed   .* 2^16); % SOMETHING WRONG HERE
expRescaleG = uint16(expDoubleG ./ maxGreen .* 2^16); % SOMETHING WRONG HERE

clear exp expG expDouble expDoubleG

redTiffPath   = strcat(expPath, '/red-corrected-noT0-tiff/');
greenTiffPath = strcat(expPath, '/green-corrected-noT0-tiff/');
rgbTiffPath = strcat(expPath, '/rgb-corrected-noT0-tiff/');
rgbTiffPathM = strcat(expPath, '/rgb-corrected-noT0-movie/');
